...
const ImageIndividual = ({person1,person2, name, title}) => {
  ...
  return (
    <div className="max-w-xs bg-white  rounded-lg shadow-md">
      <img className={hoverImage ? "rounded-lg" : "hidden rounded-lg"} src={person2} alt=""  onMouseLeave={()=>setHoverImage(false)}/>
      <img className={hoverImage ? "hidden rounded-lg" : "rounded-lg"} src={person1} alt="" onMouseEnter={()=>setHoverImage(true)}/>
